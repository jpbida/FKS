...
short mol_disti(Molecule mol1, Molecule mol2){
	double dist = 0;
	dist = sqrt( pow((mol1.x-mol2.x),2)+pow((mol1.y-mol2.y),2));
...
void Molecule::rotate2d_col_detect(std::vector<Molecule *> mols,double ang,double x2,double y2)
{
	using namespace std;
	...
	while(a < ang){
		x0=x3;
		y0=y3;
	x3 = xp*cos(a)-yp*sin(a) + x2;
	y3 = xp*sin(a)+yp*cos(a) + y2;
	if(col_detect(mols,x3,y3,0) > -1){
	a = ang;
	x=x0;
	y=y0;
	}else{
...
double d = sqrt(pow((x-mols[bound]->x),2)+pow((y-mols[bound]->y),2)+pow((z-mols[bound]->z),2));
				...
				int b1 = mols[bound]->col_detect(mols,x3b,y3b,0);
				int b2 = col_detect(mols,x3a,y3a,0);
...
//Moving the two molecules together
				...
				col_detect_b(mols,x1,y1,0,x2,y2,0);
				mols[bound]->rotate2d_col_detect(mols,ar,x,y);
...
new_verley(mols,num_mols);
	...
	int b3 = col_detect(mols,xr1,yr1,0);
...
//	 //cout << "Number of Molecules " << num_mols << endl; 
	 re_calc = 10; //resets the number of moves before the verley list needs to be recalculated
...
//		cout << w << endl;
		double t = (sqrt(pow((x-mols[i]->x),2) +pow((y-mols[i]->y),2)) - r - mols[i]->r); 
...
//cout << num_list << ": " << i << ":" << w << ": " << mols[i]->x << "," << mols[i]->y << "," << mols[i]->z  << " : " << t << endl;
			...
			num_list++;
	if(num_list > 499){
		cout << "Error Verley List not large enough" << endl;
	}
			}
			}
		}
