...
frac_data<-NULL
### read in sphere data ###
s1<-read.table(file=dataset)
### snap centers to grid ##
s1<-s1[s1$V5==2,]
s2<-s1[,1:3]
s2<-floor(s2/res)
names(s2)<-c("x","y","z")
### Create high resolution pixelized image out.dat ###
print("Creating 3d dataset")
system("cp all256.dat out.dat")
print("pixelizing")
pixelize(s2,radius_1)
print("pixelized")
### Creates image of accessible volume ###
system('sort out.dat -o out2.dat')
system('uniq -u out2.dat  out3.dat')
